...
public interface EscalaRepository extends CrudRepository<Escala, Long> {
	...
	@Query(value = "SELECT * from  escala e WHERE e.data like %:keyword% or e.categoria like %:keyword% or e.time_mandante like %:keyword% or e.time_visitante like %:keyword%", nativeQuery = true)
